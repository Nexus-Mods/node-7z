...
function createParser(emit) {
  ...
  var res = function(data) {
    ...
    var usableData = buffer + data.substr(0, usableSize + 1);
    buffer = data.substr(usableSize + 1);

    // drop a trailing line break, otherwise we might interpret it as an
    // empty line ending a block
    if (usableData.endsWith('\r\n')) {
      usableData = usableData.slice(0, -2);
    }
